      ...
      [FromForm] string Texto, [FromForm] int OrdemCronologica, [FromForm] List<int>? personagemIds = null)
   {
      try
      {
         ...
         return Json(new
         {
            ...
            {
               id = result.Id,
               tipoDeInstrucao = result.TipoDeInstrucao.ToString(),
               texto = result.Texto,
               ordemCronologica = result.OrdemCronologica,
               instrucoesPersonagens = result.InstrucoesPersonagens.Select(ip => new
               {
                  personagemId = ip.PersonagemId,
                  nome = ip.Personagem?.Nome,
                  showAll = ip.TipoDeParticipacao == TipoDeParticipacao.Executor && ip.PersonagemId == null,
                  showAllExcept = ip.TipoDeParticipacao == TipoDeParticipacao.Exceto
               })
            }
         });
      } catch (Exception ex)
...
#region MENTION
   ...
   public async Task<IActionResult> GetPersonagensForMentions(int cenaId) // Add cenaId parameter
   {
      ...
      var personagens = await _context.PecaPersonagem
         ...
         .Select(ps => new
            { ps.Personagem.Id, ps.Personagem.Nome })
         .ToListAsync();
